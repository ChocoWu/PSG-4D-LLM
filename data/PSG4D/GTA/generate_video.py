...
gta4d_json = load_categories('./data/GTA/sailvos3d.json')
...
# def process_video(data_dict):
    ...
    if video_id not in ['fin_c_ext']:  # 'fbi_2_mcs_2', 'fam_6_mcs_1', 'fbi_1_ext', 'family_4_mcs_2', 'low_fun_int', 'maude_mcs_1', 'sol_1_mcs_1_concat', 'fin_c_ext'
        continue

    save_file = f"./data/GTA/psg4d_gta/videos/{video_id}.mp4"
    # try:
    if True:
        ...
        print(f"Processing video {video_id} with {num_frames} frames", flush=True)
        for img_id in tqdm(range(num_frames), total=num_frames):
            img_name = f'{str(img_id).zfill(6)}.bmp'
            mask_name = f'{str(img_id).zfill(6)}.npy'

            img_path = f'./data/GTA/psg4d_gta/images/{video_id}'
            mask_path = f'./data/GTA/psg4d_gta/masks/{video_id}/'
            ...
            psg_path = f'./data/GTA/psg4d_gta/psg/{video_id}'
            os.makedirs(psg_path, exist_ok=True)
            cv2.imwrite(f'{psg_path}/{str(img_id).zfill(6)}.jpg', overlay_img_np)
